...
* ```Clients```
* ```WebContext``` as [RequestScoped](/apidocs/RequestedScoped.html)
* [auth filter](/apidocs/Route.Filter.html) per ```Client```
* [auth callback](/apidocs/Route.Filter.html)
...
[Auth](/apidocs/pack4j/Auth.html) module, like:
</p>
...
[Jooby](http://jooby.org) relies on [AuthStore](/apidocs/pac4j/AuthStore.html) for saving and retrieving an ```UserProfile```. By default,
...
Custom [AuthStore](/apidocs/pac4j/AuthStore.html) is provided via [Auth#store](/apidocs/pac4j/Auth.html) method:
...
from [AuthStore](/apidocs/pac4j/AuthStore.html) by calling the [AuthStore#unset](/apidocs/pac4j/AuthStore.html) method. The default login
will redirect to ```/```.
