...
public class Signature {
  ...
  private Set<String> expand(final ClassLoader classLoader, final String owner) {
    try {
      return expand(classLoader.loadClass(owner.replace("/", ".")));
  ...
  private Set<String> expand(final Class<?> owner) {
    Set<String> owners = new LinkedHashSet<>();
    expand(owner, owners);
    return owners;
  }

  private void expand(final Class<?> owner, final Set<String> owners) {
    if (owner != null && owner != Object.class) {
      owners.add(Type.getInternalName(owner));
      expand(owner.getSuperclass(), owners);
