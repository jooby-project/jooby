...
import static io.jooby.javadoc.JavaDocStream.tokens;
import static io.jooby.javadoc.JavaDocSupport.*;
import static io.jooby.javadoc.Utils.throwingFunction;
...
public class JavaDocParser {
  ...
  private int scripts(
    ...
    for (var script : tree(scope).filter(tokens(TokenTypes.METHOD_CALL)).toList()) {
      if (visited.add(script)) {
        ...
        if (METHODS.contains(callName.toUpperCase())) {
          pathLiteral(script)
              .ifPresent(
                  pattern -> {
  ...
  private ScriptRef resolveFromMethodRef(ClassDoc classDoc, DetailAST methodRef) {
    ...
    if (referenceOwner.equals("this")) {
      ...
      // resolve className
      className = toQualifiedName(classDoc.getNode(), referenceOwner);
            ...
            .filter(
                it ->
                    tree(it)
                        ...
                        .filter(
                            type ->
                                toQualifiedName(classDoc.getNode(), getQualifiedName(type))
                                    .equals("io.jooby.Context"))
                        .isPresent())
            .findFirst()
            .orElseThrow(
                () ->
  ...
  private String computePath(String prefix, String pattern) {
    ...
    return noTrailingSlash(normalizePath(prefix + pattern));
  }
   ...
   */
  private static String noTrailingSlash(String path) {
