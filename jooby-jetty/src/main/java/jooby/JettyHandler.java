...
class JettyHandler extends AbstractHandler {
  ...
  public JettyHandler(final RouteHandler handler) {
    this.handler = requireNonNull(handler, "The handler is required.");
  }
  ...
  public void handle(final String requestURI, final Request baseRequest,
    ...
    try {
      handler.handle(req.getMethod().toUpperCase(), requestURI, req.getParameterMap(),
           ...
           */
          (request, selector, interceptors, charset, produces)
          -> new JettyResponse(res, request, selector, interceptors, charset, produces));
