...
public class Issue27 extends ServerFeature {

  {
    ...
    get("/config", req -> View.of("org/jooby/hbs/issues/27/config", new Object()));

    get("/req", req -> View.of("org/jooby/hbs/issues/27/req", new Object()));

    get("/session", req -> {
      req.session().set("attr", "session-attr");
      return View.of("org/jooby/hbs/issues/27/session", new Object());
    });
  }
