  ...
  void noAcceptAndContentTypeHeaders_shouldProduceProblemAsText(ServerTestRunner runner) {
    runner.use(App::new).ready(http -> {
      ...
      var resp = given().spec(GENERIC_SPEC)
          ...
          .extract().asString();
      assertThat(resp, matchesRegex("HttpProblem\\{timestamp='.*', type=about:blank, title='Id may not be null', status=422, detail='null', instance=null\\}"));
    });
  }
  ...
  void throwMethodNotAllowedException_shouldBeTransformedToProblem(ServerTestRunner runner) {
    runner.use(App::new).ready(http -> {
      given().spec(GENERIC_SPEC)
          ...
          .body("detail", equalTo("HTTP method 'GET' is not allowed. Allowed methods are: [POST]"));
    });
  }
  ...
  void throwNotFoundException_shouldBeTransformedToProblem(ServerTestRunner runner) {
    runner.use(App::new).ready(http -> {
      given().spec(GENERIC_SPEC)
          ...
          .body("detail", equalTo("Route '/not-found' not found. Please verify request path"));
    });
  }
