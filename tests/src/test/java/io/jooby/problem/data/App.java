...
public class App extends Jooby {

  {
    Config problemDetailsConfig =
        ConfigFactory.parseMap(
            Map.of(
                ...
                true,
                "problem.details.log4xxErrors",
                true,
                "problem.details.muteCodes",
                List.of(405, 406),
                "problem.details.muteTypes",
    ...
    get(
        "/throw-http-problem-with-details",
        ctx -> {
    ...
    get(
        "/throw-problem-with-empty-builder",
        ctx -> {
          throw HttpProblem.builder().build();
        });

    get(
        "/throw-problem-with-builder-status-only",
        ctx -> {
          throw HttpProblem.builder().status(StatusCode.UNPROCESSABLE_ENTITY).build();
        });

    get(
        "/throw-problem-with-builder-title-only",
        ctx -> {
          throw HttpProblem.builder().title("Only Title").build();
        });

    get(
        "/throw-problem-with-builder-all-parameters",
        ctx -> {
    ...
    get(
        "/throw-status-code-exception-with-message",
        ctx -> {
    ...
    get(
        "/throw-number-format-exception-with-message",
        ctx -> {
    ...
    get(
        "/throw-not-acceptable-exception",
        ctx -> {
          return ctx.render(Map.of("say", "hi"));
        });

    get("/throw-unsupported-media-type", ctx -> ctx.send("hello")).consumes(MediaType.xml);

    get(
        "/throw-my-custom-exception-with-custom-render",
        ctx -> {
          throw new MyCustomException("Demo MyCustomException");
        });

    get(
        "/throw-my-custom-exception-and-propagate-as-problem",
        ctx -> {
          throw new MyCustomExceptionToPropagate("Should be propagated");
        });

    get(
        "/throw-inherited-out-of-stock-problem",
        ctx -> {
    ...
    error(
        MyCustomExceptionToPropagate.class,
        (ctx, cause, code) -> {
