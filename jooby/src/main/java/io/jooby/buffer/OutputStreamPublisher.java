...
/**
 * Bridges between {@link OutputStream} and {@link java.util.concurrent.Flow.Publisher
 ...
 * @since 6.1
 */
final class OutputStreamPublisher implements Flow.Publisher<DataBuffer> {
  ...
  @Override
  public void subscribe(Flow.Subscriber<? super DataBuffer> subscriber) {
    // We don't use Assert.notNull(), because a NullPointerException is required
    // for Reactive Streams compliance.
    Objects.requireNonNull(subscriber, "Subscriber must not be null");

    OutputStreamSubscription subscription =
        new OutputStreamSubscription(
  ...
  private static final class OutputStreamSubscription extends OutputStream
      implements Flow.Subscription {

    private static final Object READY = new Object();

    private final Flow.Subscriber<? super DataBuffer> actual;
    ...
    OutputStreamSubscription(
        Flow.Subscriber<? super DataBuffer> actual,
        Consumer<OutputStream> outputStreamConsumer,
        DataBufferFactory bufferFactory,
        int chunkSize) {
