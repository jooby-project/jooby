...
javadoc:Context[query, java.lang.Class] method:
...
The javadoc:Formdata[Form API] consist of couple of utility context methods:

- javadoc:Context[form]: `Formdata` object.

- javadoc:Context[form, java.lang.String]: retrieves a value for the 
given parameter name. Values are decoded according to the https://www.ietf.org/rfc/rfc3986.txt[RFC 3986] section 2.

- javadoc:Context[form, java.lang.Class]: creates an instance of the
given type and inject all matching parameters. Values are decoded according to the 
https://www.ietf.org/rfc/rfc3986.txt[RFC 3986] section 2.
...
The javadoc:Multipart[Multipart API] consist of couple of utility context methods:

- javadoc:Context[multipart]: `Multipart` object.

- javadoc:Context[multipart, java.lang.String]: retrieves a value for the 
given parameter name. Values are decoded according to the https://www.ietf.org/rfc/rfc3986.txt[RFC 3986] section 2.

- javadoc:Context[multipart, java.lang.Class]: creates an instance of the
given type and inject all matching parameters. Values are decoded according to the 
https://www.ietf.org/rfc/rfc3986.txt[RFC 3986] section 2.

- javadoc:Context[files]: All file uploads.

- javadoc:Context[files, java.lang.String]: Zero or more file uploads matching the given name.

- javadoc:Context[file, java.lang.String]: A file upload matching the given name.
...
<1> form variable `id`  : `root`
<2> form variable `pass`: `pwd`
<3> file upload   `pic` : javadoc:Upload[upload] object
<4> multipart object:   : `User(id=root, pass=pwd, pic=profile.png)`
<5> multipart map       : `{id=root, pass=pwd}` 

===== File Upload

As showed it before javadoc:Upload[text=file upload] are available via:
...
File `uploads` are *only* available on `multipart/form-data` request. The javadoc:Upload[Upload API]
consists of these methods:

- javadoc:Upload[filename]: Represents the file name submitted by client
- javadoc:Upload[contentType] Represent the `mime type` of the uploaded file
- javadoc:Upload[filesize] File length/size
- javadoc:Upload[path] File location as `java.nio.file.Path`
