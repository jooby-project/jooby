...
Alternative you can install a full application into a single one using the javadoc:Jooby[install, io.jooby.Jooby] operator:
...
This operator let you to deploy for example `Foo` as standalone application or integrated into a main one `App`.
Install operator shared the state of the main application, so lazy initialization of child application is *required*.
...
Dynamic routing looks similar to <<composition, composition>> but enabled/disabled routes at runtime
using a `predicate`. 
