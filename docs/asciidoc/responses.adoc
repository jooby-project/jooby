...
Jooby uses the *worker executor* to creates a coroutine context. As described in <<execution-model-worker-executor, worker executor section>>
...
Coroutines always run in the <<execution-model-worker-executor, worker executor>>. There is an experimental API where
