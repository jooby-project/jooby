...
Run database migrations on startup and exposes a [Flyway](http://flywaydb.org/)
 instance.
...
Previous example will connect to the ```DataSource``` exposed by the [jdbc](https://github.com/jooby-project/jooby/tree/master
/jooby-jdbc) module
and run the ```migrate``` command on startup. This is the recommend way of using [Flyway](http://flywaydb.org/)
, there
...
[Flyway](http://flywaydb.org/)
 looks for migration scripts at the ```db/migration``` classpath location.
...
## commands
It is possible to run [Flyway](http://flywaydb.org/)
 commands on startup, default command is: ```migrate```.
...
For more information, please visit the [Flyway](http://flywaydb.org/)
 site.
